...
def run_deployment(config: str, min_accuracy: float):
    ...
    print(
        ...
        "There you'll also be able to compare."
    )

    existing_services = mlflow_model_deployer_component.find_model_server(
        pipeline_name="continuous_deployment_pipeline", 
        pipeline_step_name="mlflow_model_deployer_step",  # Fixed typo here
        model_name="model"
    )
    ...
    if existing_services:
        ...
        if service.is_running:
            print(
                f"The MLflow prediction server is running locally as a process service "
        ...
        print(
            "No MLflow prediction server is currently running. The deployment pipeline must "
            "run first to train a model and deploy it. Execute the same command with the '--deploy' "
            "argument to deploy a model."
        )
